...
import { SANITY_STUDIO_URL } from "@/constants/config";
...
function VisualEditingInner({ client }: { client: SanityClient }) {
    ...
    () => ({
      ...
      update(update) {
        switch (update.type) {
          ...
          case "replace":
            return routerRef.current.navigate({
              to: update.url,
              replace: true,
            });
          default:
  ...
  useLiveMode({
    client,
    allowStudioOrigin: SANITY_STUDIO_URL || STUDIO_BASEPATH,
  });
        ...
        if (response.ok) {
          ...
          if (token) {
            const client = createClient({
              ...
              stega: {
                enabled: true,
                studioUrl: SANITY_STUDIO_URL || STUDIO_BASEPATH,
              },
            });
          ...
          console.warn(
            "[VisualEditing] Could not fetch draft token:",
            response.status,
          );
        }
      } catch (error) {
